...
def preorder_traversal(idx):
    ...
    if idx <= N:
        # 전위순회는 부모 노드 먼저 조사한다.
        print(tree[idx], end=' ')
        # 이제 왼쪽 자식 subtree에 대해 동일한 조건
        preorder_traversal(idx*2) # 왼쪽 자식
        # 이제 오른쪽 subtree에 대해서도 동일한 조건
...
def inorder_traversal(idx):
    # 중위 순회란,
    # 부모노드 차례가 중간인 순회 방식
    # 즉, 왼쪽 subtree에 대한 처리가 우선 되어야 한다.

    # 어디까지 순회해야 하나?
    # 순회 대상이 범위를 벗어나지 않았다면
    if idx <= N:
        # 왼쪽 자식 subtree에 대해 동일한 조건
        inorder_traversal(idx * 2)  # 왼쪽 자식
        # 중위순회는 왼쪽 서브트리 순회 후에  조사한다.
        print(tree[idx], end=' ')
        # 이제 오른쪽 subtree에 대해서도 동일한 조건
...
def postorder_traversal(idx):
    if idx <= N:
        # 왼쪽 자식 subtree에 대해 동일한 조건
        postorder_traversal(idx * 2)  # 왼쪽 자식
        # 이제 오른쪽 subtree에 대해서도 동일한 조건
...
print('후위 순회')
postorder_traversal(1)  # 'D' 'E' 'B' 'C''A'z