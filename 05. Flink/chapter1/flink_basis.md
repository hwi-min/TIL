...
- 리소스 관리 문제 -> 파이프라인 병렬 처리 (작업을 분산시킨다! 분산처리)
...
- 트위터의 Storm, Yahoo의 S3 같은 분산 스트림 병렬 처리 시스템이 개발됨
...
- **Job Manager**
  - 받아온 Job graph(논리적 구조)를 실제 Execution Graph(물리적 DAG) 형태로 만들어서 각 작업을 Task Manager에게 분배
